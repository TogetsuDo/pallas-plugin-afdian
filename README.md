...
- `绑定画画共享额度`：在已启用且列入白名单的群内，将本群共享额度绑定到自己；绑定人须已有或曾有额外额度。
- `解绑画画共享额度`：在群内解除本群共享额度绑定，仅绑定人可用。
- `画画群绑定 <gid> <qq>` / `画画群解绑 <gid>`：超管私聊为指定群绑定或解除共享额度；目标群须已启用并列入白名单。
- `用户额度 <qq>`：超管私聊查看指定 QQ 的额外额度。
- `用户额度设置 <qq> <n>`：超管私聊将指定 QQ 的额外额度设为整数次数。
